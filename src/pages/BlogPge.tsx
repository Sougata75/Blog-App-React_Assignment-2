            ...
            {filteredData.map((blog) => (
              <div key={blog.id} className="w-full flex flex-wrap justify-between px-2 md:px-0">
                ...
                <div className="w-full pt-4 md:pt-0 md:w-[16%] flex justify-center items-center">
                  <button onClick={()=> navigate(`/blogPost/${blog.id}`)} className="text-white flex gap-2 border border-gray-500 px-[37.5%] md:px-8 py-2 md:py-3 rounded-md md:rounded-sm md:hover:translate-y-[-2px] transition-all">View <ArrowUpRight color="#eab308"/></button>
                </div>
              </div>
            ))}
